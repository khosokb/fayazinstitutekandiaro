...
# ----------------- Login Page -----------------
    ...
    if st.session_state.get("logged_in"):
        ...
        if admission_record:
            ...
            st.write(f"**Admission No:** {admission_record.get('admission_no')}")
            st.write(f"**Course:** {', '.join(admission_record.get('courses', []))}")
            st.write(f"**Status:** {admission_record.get('status')}")
            ...
            st.info("No admission record found. Fill the Admission Form.")
...
with st.expander("Reset Password"):
    rp_username = st.text_input("Enter your Registered Username", key="rp_user")
    rp_cnic = st.text_input("Enter Your CNIC (without dashes)", key="rp_cnic")
    new_pass = st.text_input("New Password", type="password", key="rp_np")
    new_pass2 = st.text_input("Confirm New Password", type="password", key="rp_np2")
    ...
    if st.button("Reset My Password"):
        ...
        elif users[rp_username]["cnic"] != rp_cnic:
            st.error("❌ CNIC does not match our records.")
        elif new_pass != new_pass2:
            st.error("❌ Passwords do not match.")
        else:
            # Update password
            users[rp_username]["password"] = make_hash(new_pass)
            save_json(USERS_FILE, users)
...
# ----------------- Admin Panel Page -----------------
    ...
    if st.session_state.get("admin_logged_in"):
            ...
            for adm_no, rec in studs.items():
                ...
                if cols[2].button(f"Create User {adm_no}", key=f"cu-{adm_no}"):
                    ...
                    if uname in users:
                        ...
                        st.success(f"User created. Default password is student's mobile number.")

                # PDF Download
                if cols[3].button(f"PDF {adm_no}", key=f"pdf-{adm_no}"):
        ...
        if st.button("Download All Students CSV"):
            ...
            writer.writerow([
                "Admission No", "Full Name", "Father Name", "Mobile",
                "Courses", "Status", "Applied At"
            ])

            for adm_no, rec in studs.items():
                writer.writerow([
                    rec.get("admission_no"),
                    rec.get("full_name"),
                    rec.get("father_name"),
                    rec.get("contact_no"),
                    "; ".join(rec.get("courses", [])),
                    rec.get("status"),
                    rec.get("applied_at"),
                ])
