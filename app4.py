...
ALUMNI_DIR = os.path.join(UPLOAD_DIR, "students")  # Alumni photos
TEACHERS_DIR = os.path.join(UPLOAD_DIR, "teachers")  # Teachers photos
...
def save_uploaded_file(uploaded_file, admission_no: str, folder=UPLOAD_DIR):
...
if menu == "Home":
    ...
    st.image("https://images.unsplash.com/photo-1522071820081-009f0129c71c",
             caption="Learn — Practice — Succeed", use_column_width=True)
    show_footer()
            ...
            if st.button("Download Admission PDF"):
                ...
                with open(pdf_path, "rb") as f:
                    st.download_button("Download PDF", data=f,
                                       file_name=f"{admission_record['admission_no']}.pdf",
...
# ----------------- Teachers / Gallery -----------------
    ...
    st.header("Teachers Gallery")
    teacher_photos = glob.glob(os.path.join(TEACHERS_DIR, "*.jpg"))
    if teacher_photos:
        cols = st.columns(4)
        for i, photo_path in enumerate(teacher_photos):
            with cols[i % 4]:
                st.image(photo_path, use_column_width=True)
                st.caption(os.path.splitext(os.path.basename(photo_path))[0].replace("_", " "))
    else:
        st.info("No teacher photos uploaded yet.")

# ----------------- Alumni Gallery -----------------
elif menu == "Alumni Gallery":
    st.header("Alumni Gallery")
    alumni_photos = glob.glob(os.path.join(ALUMNI_DIR, "*.jpg"))
    if alumni_photos:
        cols = st.columns(4)
        for i, photo_path in enumerate(alumni_photos):
            with cols[i % 4]:
                st.image(photo_path, use_column_width=True)
                st.caption(os.path.splitext(os.path.basename(photo_path))[0].replace("_", " "))
    else:
        st.info("No alumni photos uploaded yet.")
                ...
                if cols[2].button(f"Create User {adm_no}", key=f"cu-{adm_no}"):
                    ...
                    if uname in users:
                        ...
                        save_user(uname, {
                            ...
                            "password": make_hash(str(pwd)),
                            "created_at": str(datetime.datetime.now())
                        })
                        st.success(f"User created with password: {pwd}")
                if cols[3].button(f"Download PDF {adm_no}", key=f"pdf-{adm_no}"):
                    pdf_path = generate_admission_pdf(rec, f"{adm_no}.pdf")
                    with open(pdf_path, "rb") as f:
                        st.download_button("Download PDF", data=f,
                                           file_name=f"{adm_no}.pdf",
                                           mime="application/pdf")

        st.write("---")
        st.subheader("Upload Gallery Photos")
        colA, colB = st.columns(2)
        with colA:
            alumni_name = st.text_input("Alumni Name")
            alumni_photo = st.file_uploader("Upload Alumni Photo", type=["jpg","jpeg","png"], key="alumni_upload")
            if st.button("Upload Alumni Photo"):
                if alumni_photo and alumni_name:
                    filename = f"{alumni_name.replace(' ', '_')}.jpg"
                    path = os.path.join(ALUMNI_DIR, filename)
                    with open(path, "wb") as f:
                        f.write(alumni_photo.getbuffer())
                    st.success(f"Alumni photo uploaded: {filename}")
                else:
                    st.error("Enter name and select photo.")
        with colB:
            teacher_name = st.text_input("Teacher Name")
            teacher_photo = st.file_uploader("Upload Teacher Photo", type=["jpg","jpeg","png"], key="teacher_upload")
            if st.button("Upload Teacher Photo"):
                if teacher_photo and teacher_name:
                    filename = f"{teacher_name.replace(' ', '_')}.jpg"
                    path = os.path.join(TEACHERS_DIR, filename)
                    with open(path, "wb") as f:
                        f.write(teacher_photo.getbuffer())
                    st.success(f"Teacher photo uploaded: {filename}")
                else:
                    st.error("Enter name and select photo.")
